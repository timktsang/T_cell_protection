...
plotfunction <- function(data1,titlename,pa,whichrow,trun){
  ...
  alldata <- data1[data1$season==whichrow-1,7+2*(0:3)]  
  ...
  for (j in 1:4){
    ...
    plotvec <- pmin(trun*1.00,data1[data1$season==whichrow-1&data1$inf==1,7+2*(j-1)])
    points( jitter(rep(j-0.5+dif1,length(plotvec)),amount=jpara1), plotvec,col=rgb(1,0,0,sd2),pch=16)
    plotvec <- pmin(trun*1.00,data1[data1$season==whichrow-1&data1$inf==0,7+2*(j-1)])
  ...
  mtext(expression(paste("CD69"^"+","IFN-",gamma^"+"," in CD4 (%)",sep="")),side=2,line=2.25)
  
  mtext(titlename,side=3,line=0.5)
  title(main=pa, adj=0)

  if (whichrow==6){
    legend(2,0.9,c("Infection","Non-infection"),pch=16:17,lty=1,cex=1,bty="n",col=c("red","blue"))
  }  
}

pdf("Shared kiddivax CMI/protection/figureS2.pdf",width=9, height=6)
layout(matrix( 1:6, nrow=2,byrow=T))
plotfunction(data1,"Epidemic 1: pH1N1","A",1,1)
plotfunction(data1,"Epidemic 2: H3N2","B",2,1)
plotfunction(data1,"Epidemic 3: pH1N1","C",3,1)
plotfunction(data1,"Epidemic 4: H3N2","D",4,3)
plotfunction(data1,"Epidemic 5: pH1N1","E",5,1)
plotfunction(data1,"Epidemic 6: H3N2","F",6,1)
